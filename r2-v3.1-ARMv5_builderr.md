...
Details of Tested Platform & OS: (worked with previous r2 version)
```
...
```
Workaround: (use this ONLY until permanent fix is ready)
```asm
tweaking `libr/include/r_util/r_sys.h` by changing the `bkpt` value (worked)
```

Compile log: (was using older version for comparison/double-check)
```asm
$ sys/install.sh
